...
dtime = 0.03
g_scal = 9.8
Fricscale = 0.001 * Mtot * g_scal##################################
g = np.array([[0], [0], [-g_scal]])
resis_scale_square = 0.0001 * Mtot*g_scal########################
...
class robot:
    ...
    def set_constants(self):
        ...
        self.body.vs = np.array([[0.],[0.],[0.]])#np.zeros((3, 1), dtype=np.float64)
        self.body.wb = np.array([[0.],[0.],[0.]])#np.zeros((3, 1), dtype=np.float64)
    ...
    def theta_done(self):
        for p in range(self.numLeg):
            for i in range(self.numsubleg):
                if (self.leg[p].sub[i].theta > math.pi*5/4.):
                    return 1
                elif (self.leg[p].sub[i].theta < -math.pi*5/4.):
                    return 1
                return 0
    ...
    def timeflow(self):
        ...
        if(wbnorm != 0):
            Rwb = rotate_axis(self.body.wb/wbnorm, np.linalg.norm(self.body.wb)*dtime)
            self.body.Q = self.body.Q.dot(Rwb)
            ####################################################################################################################################
